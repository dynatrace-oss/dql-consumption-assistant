...
import useFetchNotebookQueries from "./useFetchNotebookQueries";
import useGiveTableColumns from "./useGiveTableColumns";
import type { NotebooksQueriesResultType } from "./types";

const NotebooksQueries: React.FC = () => {
  const { columns } = useGiveTableColumns();

  const { data, isLoading, isError, error, isSuccess, refetch } =
    useFetchNotebookQueries();

  const intl = useIntl();

  const expandableRowsJsx = (rowData: NotebooksQueriesResultType) => {
    return (
...
export default NotebooksQueries;
